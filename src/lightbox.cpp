...
Vec3d trace(Vec3d ray_origin, Vec3d ray, int depth) {
  if(depth == 0) {
    ...
    L = normalize(Vec3d(rrand()*2-1, rrand()*2-1, rrand()*2-1));
...
int main(int argc, char** argv) {
  ...
  spheres[1].center = Vec3d(0.,0., -.9);
  spheres[1].albedo = Vec3d(.2, .7, .4);
  spheres[1].radius = 0.3;

  spheres[2].center = Vec3d(.85, -.2, -2.6);
  spheres[2].albedo = Vec3d(.7, .5, .4 );
  spheres[2].radius = .8;

  plight.center = Vec3d(-3.,10.,5.);
  plight.albedo = Vec3d(1.,1.,0.);

  ground.point = Vec3d(0., -.1, 0.);
  ground.norm = normalize(Vec3d(0.,1.,0.));
  ground.albedo = Vec3d(.2, 0.8, 0.4);
